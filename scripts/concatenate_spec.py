...
for k,val in enumerate(LR):
    decorr.concatenate_spec('spec/gaussian/synch_therm_{:s}_mcplusexcess_noise_xpol_[0-9]???.pickle'.format(val))
    decorr.concatenate_spec('spec/gaussian/synch_therm_{:s}_qucov_noise_xpol_[0-9]???.pickle'.format(val))
    #decorr.concatenate_spec('spec/gaussian/synch_therm_{:s}_qucovds_noise_[0-9]???.pickle'.format(val))
